...
public class Parser {
    ...
    public ExpTree parseLine() {
        ...
        if (lex.token == Lexer.let) {
            lex.getToken();
            defs = parseDefList();
            if (lex.token != Lexer.in) throw new ParseException("'in' expected");
            lex.getToken();
        }
        ExpTree result = parseExp(true);
        if (lex.token != Lexer.semico) throw new ParseException("semicolon expected");
        if (defs != null)
    ...
    static ExpTree makeDefTree(String s, ExpTree t) {
        return new ExpTree(ExpTree.eqNode, s, makeIdLeaf(s), t);
